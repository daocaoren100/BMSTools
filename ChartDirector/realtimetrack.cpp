...
//    setWindowTitle("Realtime Chart with Track Line");
    ...
    // MAX_V Value display
    (new QLabel("MAX_V", frame))->setGeometry(6, 177, 48, 21);
    m_ValueA = new QLabel(frame);
    m_ValueA->setGeometry(55, 177, 59, 21);
    m_ValueA->setFrameShape(QFrame::StyledPanel);

    // MIN_V Value display
    (new QLabel("MIN_V", frame))->setGeometry(6, 200, 48, 21);
    m_ValueB = new QLabel(frame);
    m_ValueB->setGeometry(55, 200, 59, 21);
    m_ValueB->setFrameShape(QFrame::StyledPanel);

    // MAX_T Value display
    (new QLabel("MAX_T", frame))->setGeometry(6, 223, 48, 21);
    m_ValueC = new QLabel(frame);
    m_ValueC->setGeometry(55, 223, 59, 21);
    m_ValueC->setFrameShape(QFrame::StyledPanel);

    // MIN_T Value display
    (new QLabel("MIN_T", frame))->setGeometry(6, 246, 48, 21);
    m_ValueD = new QLabel(frame);
    m_ValueD->setGeometry(55, 246, 59, 21);
...
//
void RealtimeTrack::getData(double dataA, double dataB, double dataC, double dataD)
{
    // The current time
...
//        double dataC = 150 + 100 * cos(p / 6.7) * cos(p / 11.9);
    ...
    //
    m_ValueA->setText(QString::number(m_dataSeriesA[m_currentIndex - 1], 'f', 3));
    m_ValueB->setText(QString::number(m_dataSeriesB[m_currentIndex - 1], 'f', 3));
    m_ValueC->setText(QString::number(m_dataSeriesC[m_currentIndex - 1], 'f', 1));
    m_ValueD->setText(QString::number(m_dataSeriesD[m_currentIndex - 1], 'f', 1));
}
...
void RealtimeTrack::drawChart()
{
    ...
    XYChart *c = new XYChart(700, 270, 0xf4f4f4, 0x000000, 1);
    ...
    c->setPlotArea(55, 55, 590, 185, 0xffffff, -1, -1, 0xcccccc, 0xcccccc);
    c->setClipping();
    ...
    c->yAxis()->setTitle("Volt (V)", "arialbd.ttf", 10);

    // Configure the y-axis with a 10pts Arial Bold axis title
    c->yAxis2()->setTitle("Temp (C)", "arialbd.ttf", 10);
    ...
    if (firstTime != Chart::NoValue)
    {
        ...
        layer->addDataSet(DoubleArray(m_dataSeriesA, sampleSize), 0xff0000, "MAX_V");
        layer->addDataSet(DoubleArray(m_dataSeriesB, sampleSize), 0x00cc00, "MIN_V");
        //layer->addDataSet(DoubleArray(m_dataSeriesC, sampleSize), 0x0000ff, "MAX_T")->setUseYAxis2();
        layer->addDataSet(DoubleArray(m_dataSeriesD, sampleSize), 0xffff00, "MIN_T")->setUseYAxis2();
	}
